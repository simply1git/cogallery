...
const authenticateJWT = async (req, res, next) => {
  // Apply rate limiting to authentication attempts BEFORE token validation
  const authLimiterResult = rateLimiterService.checkRateLimit(req, 'auth');
  if (!authLimiterResult.allowed) {
    res.setHeader('X-RateLimit-Limit', authLimiterResult.limit);
    res.setHeader('X-RateLimit-Remaining', authLimiterResult.remaining);
    res.setHeader('X-RateLimit-Reset', new Date(authLimiterResult.resetTime).toISOString());
    res.setHeader('Retry-After', authLimiterResult.retryAfter);

    return res.status(authLimiterResult.reason === 'banned' ? 403 : 429).json({
      error: authLimiterResult.message || 'Too many requests',
      type: authLimiterResult.reason
    });
  }
  ...
  try {
    ...
    rateLimiterService.recordFailure(req, 'invalid_token');
...
// Create middleware functions for different endpoint types
const uploadLimiter = rateLimiterService.middleware('upload');
const mediaLimiter = rateLimiterService.middleware('media');
const adminLimiter = rateLimiterService.middleware('admin');
const authLimiter = rateLimiterService.middleware('auth');
const globalLimiter = rateLimiterService.middleware('global');
...
// Graceful shutdown
const cleanup = () => {
  console.log(`[Shutdown] SIGINT or SIGTERM received`);

  // Close rate limiter
  if (rateLimiterService && typeof rateLimiterService.shutdown === 'function') {
