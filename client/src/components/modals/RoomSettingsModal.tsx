...
export function RoomSettingsModal({ isOpen, room, onClose, onUpdate }: RoomSettingsModalProps) {
  ...
  const handleSaveCrop = async (croppedBlob: Blob) => {
    ...
    const extension = cropperFile.name.split('.').pop() || 'jpg'
    const newFile = new File([croppedBlob], `cropped-cover.${extension}`, { type: `image/${extension === 'png' ? 'png' : 'jpeg'}` })
    
    const result = await uploadThumbnail(newFile, room.id)
        ...
        <div className="relative w-full max-w-md glass-md rounded-2xl p-4 sm:p-6 animate-scale-in max-h-[90vh] overflow-y-auto">
          ...
          <div className="mb-6 flex flex-col items-center">
            ...
            >
              {thumbnailUrl ? (
                <>
                  <img src={thumbnailUrl} alt="Room Cover" className="w-full block" />
                  <button
                    type="button"
