        ...
        for (let i = 0; i < fileIDs.length; i += concurrencyLimit) {
          ...
          await Promise.all(batch.map(async (fileID: string) => {
            ...
            if (isVault) {
               ...
               // Elite Implementation: Zero-Memory Stream Encryption pointer
               // We don't read the stream into a chunk array! We let encryptFile create a Blob pointer.
               const { encryptFile } = await import('@/services/cryptoService');
               const encryptedBlob = await encryptFile(payloadToUpload, vaultKey);
               
               const currentName = (payloadToUpload as File).name || file.name;
               // Uppy can use Blobs natively. We just spoof the name property.
               (encryptedBlob as any).name = currentName;
               payloadToUpload = encryptedBlob;
  ...
  return (
    <div className="uppy-dashboard-container rounded-2xl overflow-hidden border border-white/[0.08] shadow-2xl relative">
      <Dashboard
        ...
        note="Unlimited file size. Resumable uploads. Automatic offline recovery."
      />
    </div>
  )
}