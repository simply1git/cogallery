...
class AuditLogger {
  constructor(options = {}) {
    ...
    this.store = []; // In-memory store for simplicity (in production, use append-only DB or WORM storage)
    this.chainId = null; // Hash of previous entry for chaining
    this.maxSize = options.maxSize || 10000; // Maximum entries before rolling over
  ...
  _createGenesisBlock() {
    const genesisBlock = {
      ...
      data: { action: 'SYSTEM_START', message: 'Audit log initialized' },
      previousHash: '0'.repeat(64),
      hash: this._calculateHash(0, new Date().toISOString(), { action: 'SYSTEM_START', message: 'Audit log initialized' }, '0'.repeat(64))
    };
